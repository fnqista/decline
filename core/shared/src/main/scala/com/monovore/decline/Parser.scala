...
private[decline] case class Parser[+A](command: Command[A])
        ...
        accumulator.parseOption(Opts.LongName(option)) match {
          case Some(MatchFlag(_)) =>
            ConsumeResult.parsed(
              accumulator.result.failWithError(Messages.unexpectedValueForFlag(s"--$option"))
            )
          case Some(MatchAmbiguous) =>
            ConsumeResult.parsed(
              accumulator.result.failWithError(Messages.ambiguousOptionFlag(s"--$option"))
            )
          ...
          case None =>
            mode match {
              case ParseMode.Strict =>
                ConsumeResult.parsed(
                  accumulator.result.failWithError(Messages.unexpectedOption(s"--$option"))
                )
        ...
        accumulator.parseOption(Opts.LongName(option)) match {
          ...
          case Some(MatchOption(next)) =>
            rest match {
              case Nil =>
                ConsumeResult.parsed(
                  accumulator.result.failWithError(Messages.missingValueForOption(s"--$option"))
                )
            ...
            toOption(accumulator.parseArg(arg)) match {
              ...
              case None =>
                mode match {
                  case ParseMode.Strict =>
                    ConsumeResult.parsed(
                      accumulator.result.failWithError(Messages.unexpectedArgument(arg))
                    )
                  case ParseMode.Lenient =>
                    consumeAll(
                      rest,
  ...
  private[this] def consumeShort(
    ...
    accumulator.parseOption(Opts.ShortName(char)) match {
      case Some(MatchAmbiguous) =>
        Left(accumulator.result.failWithError(Messages.ambiguousOptionFlag(s"-$char")))
      ...
      case Some(MatchOption(next)) =>
        tail match {
          case "" =>
            rest match {
              case Nil =>
                Left(accumulator.result.failWithError(Messages.missingValueForOption(s"-$char")))
      ...
      case None =>
        mode match {
          case ParseMode.Strict =>
            Left(accumulator.result.failWithError(Messages.unexpectedOption(s"-$char")))
          case ParseMode.Lenient =>
            tail match {
              case "" =>
