...
class Command[+A] private[decline] (
  ...
  /**
   * Parse arguments in lenient mode - unexpected options/args are accumulated as warnings.
   * Returns Ior.Left if parsing failed with errors.
   * Returns Ior.Right if parsing succeeded with no warnings.
   * Returns Ior.Both if parsing succeeded but there were warnings (unexpected options/args).
   */
